"""
Analytics Dashboard for Blood on the Clocktower (BOTC) games run by Matan Diamond.
...
focussed exclusively on games where the storyteller is ``Matan_Diamond``.
...
* **Player statistics:** aggregated win rates for each player across all
  Matan Diamond games. For every player you can see their total games,
...
class MatanAnalytics:
    ...
    games : list of dict
        ...
        ``story_teller`` field matches ``TARGET_STORYTELLER`` are considered.
...
class AnalyticsUI:
    ...
    def __init__(self, root: tk.Tk, analytics: MatanAnalytics) -> None:
        self.root = root
        self.analytics = analytics
        root.title(f"BOTC Analytics - {TARGET_STORYTELLER}")
...
def main() -> None:
    ...
    analytics = MatanAnalytics(gamelog)
    root = tk.Tk()
